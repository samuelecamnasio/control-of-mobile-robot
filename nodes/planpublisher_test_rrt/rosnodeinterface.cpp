...
// Generated 15-Jul-2021 19:04:05
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"planPublisher_test_RRT\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(planPublisher_test_RRT_M, (NULL));
        planPublisher_test_RRT_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(planPublisher_test_RRT_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(planPublisher_test_RRT_M)
                == (NULL)) || rtmGetStopRequested(planPublisher_test_RRT_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        planPublisher_test_RRT_terminate();
        mNode.reset();
      }
    }
      ...
      msg)
    {
      ...
      const uint64_t baseRateNanoSec = 200000000;
      ...
      if (false == waitingFlag) {
        if (0U == msg.clock.nsec) {
          ...
          mRunModel = (rtmGetErrorStatus(planPublisher_test_RRT_M) == (NULL));
          if (mRunModel == false) {
            termFlag = true;
          }
        }
    ...
    {
      mRunModel = (rtmGetErrorStatus(planPublisher_test_RRT_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
