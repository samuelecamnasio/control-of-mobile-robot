...
// Generated 15-Jul-2021 19:03:37
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"planPublisher_test\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(planPublisher_test_M, (NULL));
        planPublisher_test_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(planPublisher_test_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(planPublisher_test_M)
                == (NULL)) || rtmGetStopRequested(planPublisher_test_M));
      ...
      msg)
    {
      ...
      if (false == waitingFlag) {
        if (0U == msg.clock.nsec) {
          ...
          mRunModel = (rtmGetErrorStatus(planPublisher_test_M) == (NULL));
          if (mRunModel == false) {
            termFlag = true;
          }
        }
    ...
    {
      mRunModel = (rtmGetErrorStatus(planPublisher_test_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
