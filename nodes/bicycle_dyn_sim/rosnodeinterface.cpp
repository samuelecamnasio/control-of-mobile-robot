...
// Generated 15-Jul-2021 19:21:57
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"bicycle_dyn_sim\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(bicycle_dyn_sim_M, (NULL));
        bicycle_dyn_sim_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(bicycle_dyn_sim_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(bicycle_dyn_sim_M)
                == (NULL)) || rtmGetStopRequested(bicycle_dyn_sim_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(bicycle_dyn_sim_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
